...
export function getLifetimes(path: NodePath<ObjectExpression>, t: any, propName?: string): NodePath<ObjectProperty> {
  ...
  let lifetimes: NodePath<ObjectProperty> | undefined = props.find(p => p.get('key')?.isIdentifier({ name: propName }));
  ...
  let n = props.find(p => p.get('key').isIdentifier({ name: propName })) as NodePath<ObjectProperty>;
  if (!n) { debugger }
  return n;
}
...
export function getAttachByProp(path: NodePath<ObjectProperty>, t: any, attachKey?: string): NodePath<ObjectProperty> {
  if (!attachKey) attachKey = 'attached';
  let props = path.get('value')?.get('properties') as NodePath<ObjectProperty>[]
  let attached: NodePath<ObjectProperty> | undefined = props.find(p => p?.get('key')?.isIdentifier({ name: attachKey }));
  if (attached) return attached;
  if (!(path.node as any)?.value?.properties) { debugger }
  path.get('value')?.unshiftContainer('properties' as never,
    t.objectProperty(
      t.identifier(attachKey), t.functionExpression(null, [t.identifier('e')], t.blockStatement([]))
    )
  )
  props = path.get('value')?.get('properties') as NodePath<ObjectProperty>[]
  return props.find(p => p.get('key').isIdentifier({ name: attachKey })) as NodePath<ObjectProperty>;
}
...
export function getOnloadProp(path: NodePath<ObjectExpression>, t: any, onLoadKey?: string): NodePath<ObjectProperty> {
  ...
  let props = path.get('properties') as NodePath<ObjectProperty>[]
  let lifetimes: NodePath<ObjectProperty> | undefined = props.find(p => p.get('key')?.isIdentifier({ name: onLoadKey }));
  if (lifetimes) return lifetimes;
  path.unshiftContainer('properties', t.objectProperty(
    t.identifier(onLoadKey), t.functionExpression(null, [], t.blockStatement([]))
  ))
  props = path.get('properties') as NodePath<ObjectProperty>[]
  let n = props.find(p => p.get('key').isIdentifier({ name: onLoadKey })) as NodePath<ObjectProperty>;
  if (!n) { debugger }
  return n;
}
